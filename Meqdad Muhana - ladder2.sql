...
select count(age) Total_count, round(avg(age),1) Mean_of_Ages, min(age) Minimum_age, max(age) Maximum_age from pets;
...
select printf('%,d Fr.', avg(salary*0.97)) as In_CHF,
printf('$%,d', avg(salary)) as In_USD
from employees;
...
from employees
group by job;
-- 50) Show the mean, min, and max salaries of each job title in `employees`, as well as
-- the numbers of employees in each category.
select job, avg(salary), min(salary), max(salary), count(*) as employees_num
from employees
group by job;
...
-- 55) Show the top 5 largest orders (and their respective customer) in terms
-- of the numbers of items purchased in that order.
select order_id, customer, sum(quantity)
from transactions
group by order_id, customer
order by sum(quantity) desc
limit 5;
-- 56) Show the total cost of each transaction.
--     * _Hint:_ The `unit_price` column is the price of _one_ item.
--     The customer may have purchased multiple.
--     * _Hint2:_ Note that transactions here span multiple rows if different
--       items are purchased.
...
limit 5;
-- 58) Show the top 5 customers in terms of total revenue
--   (ie, which customers have we done the most business with in terms of money?)
select customer, sum(unit_price*quantity)
from transactions
group by customer
order by sum(unit_price*quantity) DESC
limit 5;
-- 59) Show the top 5 employees in terms of revenue generated
--   (ie, which employees made the most in sales?)
