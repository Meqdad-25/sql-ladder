...
-- In addition, only show employees who are _not_ in sales. This will be a little bit of a longer query.
...
select id, firstname || ', ' || lastname as fullname, job,  round(salary / 1.1, 2) as salary_eu,
...
from employees;
-- 28) Query the `employees` table, replacing `firstname`/`lastname` with `fullname` and `startdate`
-- with `startyear`. Print out the salary in USD again, except format it with a dollar sign, comma separators, 
-- and no decimal. For example, the first row should read `$123,696`. This column should still be named `salary`.
...
select id, firstname || ', ' || lastname as fullname, job, printf('$%,d', salary) as salary,
strftime('%Y',startdate) as startyear
from employees;

-- **Note:** For the next few problems, you'll probably want to use `CASE`/`WHEN` statements.
-- 
-- 29) Last year, only salespeople were eligible for bonuses.
-- Create a column `bonus` that is "Yes" if you're eligible for a bonus, otherwise "No".
select *, CASE
...
select id, firstname, lastname, job, printf('$%,d', salary) as salary,startdate, CASE
when job = 'Sales' AND salary > 100000 then printf('$%,d', salary*1.1)
when job = 'Sales' AND salary < 100000 then printf('$%,d', salary*1.05)
when job = 'Administrator' then printf('$%,d', salary*1.05)
